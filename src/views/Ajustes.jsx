...
export const Ajustes = () => {
  ...
  const getEstadoOpciones = async () => {
    ...
    desplazamiento === 'activo'
      ? setCheckOpcionDesplazamiento(true)
      : setCheckOpcionDesplazamiento(false);

    const incidente = await AsyncStorage.getItem('opcion-incidente');
    incidente === 'activo'
      ? setCheckOpcionIncidente(true)
      : setCheckOpcionIncidente(false);

    const marcador = await AsyncStorage.getItem('opcion-marcador');
    marcador === 'activo'
      ? setCheckOpcionMarcador(true)
      : setCheckOpcionMarcador(false);

    const contador = await AsyncStorage.getItem('opcion-contador');
    contador === 'activo'
      ? setCheckOpcionContador(true)
      : setCheckOpcionContador(false);
  };
  ...
  const SincronizarList = () => {
    return (
      <ListItem.Accordion
        ...
        <ListItem Component={TouchableHighlight} onPress={sincronizarCatalogos}>
          <ListItem.Content>
  ...
  const OpcionDesplazamiento = useCallback(() => {
    return (
      <ListItem>
        ...
        <ListItem.Content>
          <ListItem.Title>
            Envió automático de registros de desplazamientos
          </ListItem.Title>
        </ListItem.Content>
        <Switch
  ...
  const OpcionIncidente = useCallback(() => {
    return (
      <ListItem>
        ...
        <ListItem.Content>
          <ListItem.Title>
            Envió automático de registros de incidentes
          </ListItem.Title>
        </ListItem.Content>
        <Switch
          trackColor={{false: '#d8d8db', true: '#d8d8db'}}
  ...
  const OpcionMarcador = () => {
    return (
      <ListItem>
        ...
        <ListItem.Content>
          <ListItem.Title>
            Envió automático de registros de marcadores
          </ListItem.Title>
        </ListItem.Content>
        <Switch
          trackColor={{false: '#d8d8db', true: '#d8d8db'}}
  ...
  const OpcionContador = () => {
    return (
      <ListItem>
        ...
        <ListItem.Content>
          <ListItem.Title>
            Envió automático de registros de Contadores
          </ListItem.Title>
        </ListItem.Content>
        <Switch
          trackColor={{false: '#d8d8db', true: '#d8d8db'}}
            ...
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
                ...
                <Text
                  ...
                  Cualquier registros no sincronizado se perderá
                </Text>
                <View
                  style={{
                    flexDirection: 'row',
      ...
      <View style={styles.foobar}>
        ...
        <View
          ...
          <Text style={{...styles.text, color: 'grey'}}>
            © {fecha} Universidad de El Salvador.
          </Text>
          <Text style={{...styles.text, color: 'grey'}}>
